...
#create output file
row = ['organization', 'uuid', 'patent_id', 'assignee_id', 'rawlocation_id', 'type', 'name_first', 'name_last', 'sequence']
...
for j in range(len(df.organization)): #check if names exist in set
	...
	if isaMatch(df.organization[j]): #check if names match
		count += 1
		row = [re.sub(r'[^\x00-\x7F]+',' ', df.organization[j]), df.uuid[j], df.patent_id[j], df.assignee_id[j], df.rawlocation_id[j], df.type[j], df.name_first[j], df.name_last[j], df.sequence[j]]
...
sortedCSV = df.iloc[df.organization.str.upper().str.replace(' ', '').argsort()]
